<div id="onecat-<?php echo $D->one->idcategory?>" class="one_item">
    ...
    <div id="spacecat-<?php echo $D->one->idcategory?>">
        ...
        <div class="info_item">
            ...
            <div><span id="bedit-<?php echo $D->one->idcategory?>" class="the_option">Edit</span> <span class="the_option"><a href="<?php echo $K->SITE_URL?>admin/articles/subcategories/c:<?php echo $D->one->idcategory?>" rel="phantom" target="dashboard-main-area-right"><span class="bold"><?php echo $D->one->num_children?></span> <?php echo($D->one->num_children == 1 ? $this->lang('admin_articles_categories_text_subcategory') : $this->lang('admin_articles_categories_text_subcategories'))?></a></span></div>
        </div>
    </div>
    ...
    <div id="areaedit-<?php echo $D->one->idcategory?>" class="hide edit_item">
        ...
        <div class="space-in-admin">
            ...
            <div class="form-block">
                <label for="namecategory<?php echo $D->one->idcategory?>"><?php echo $this->lang('admin_articles_categories_text_category')?></label>
...
$('#delcat-<?php echo $D->one->idcategory?>').click(function(){
    closeEmerged();
    _confirm(msg_delete_category_article, nothign, deleteCategoryArticle, <?php echo $D->one->idcategory?>);
});
...
$('#bupdate<?php echo $D->one->idcategory?>').click(function(e){
    e.preventDefault();
    updateCategoryArticle(<?php echo $D->one->idcategory?>);
});
